...
class Spline {
  ...
  setPos(idx, pos) {
    oito.vec3.copy(pos, this.points[idx].pos);
    return this;
  }
  at(t, pos) {
    return pos || [0, 0, 0];
...
class BezierQuadSpline extends Spline {
  ...
  atCurve(cIdx, t, pos, dxdy, dxdy2) {
    ...
    const p = this.points;
    const a = cIdx * 2;
    const b = oito.Maths.mod(a + 1, this._pointCnt);
    const c = oito.Maths.mod(a + 2, this._pointCnt);
    if (pos)
  ...
  _computeLoopIdx(t) {
    ...
    return [i, oito.Maths.mod(i + 1, this._pointCnt), oito.Maths.mod(i + 2, this._pointCnt), tt];
  }
}
class Hermite {
...
exports.BezierQuadSpline = BezierQuadSpline;
